...
def fit_parabola(xdata, ydata, func, plot_file, ylim=0):
    if ylim > 0:
        reduced_x = [] 
        reduced_y = []
        for i in range(len(xdata)):
            if ydata[i] <= ylim:
                reduced_x.append(xdata[i])
                reduced_y.append(ydata[i])
        xdata = np.array(reduced_x)
        ydata = np.array(reduced_y)
    popt, pcov = curve_fit(func, xdata, ydata)
    y_fitted = func(xdata, popt[0], popt[1])
    custom_multiple_subplots(plot_file, 2, 1, [[xdata, xdata]], [[ydata, y_fitted]], ["Data", "Fitted"], ["Parabola Fit"], fig_size=(4,8), symbols=["-", "-"], detailed_ticks=False)
    print(popt)
    return popt
...
def crunch(surf_file, loss_key, acc_key, comm, rank, args):
    ...
    pops = fit_parabola(xcoordinates, losses, parabola, plot_file, ylim=0.5)
...
if __name__ == '__main__':
    ...
    if not args.disaggregated:
        crunch(surf_file, 'train_loss', 'train_acc', comm, rank, args)
