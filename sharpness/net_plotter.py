...
import torch
import copy
from os.path import exists, commonprefix, basename, dirname
...
def name_direction_file(args, sub_dir="", model_file="", seed=0):
    ...
    model_filename = basename(dir_file)
    dir_file = dirname(dir_file) + "/" + sub_dir
    if not exists(dir_file):
        mkdir(dir_file)
    if seed != 0:
        dir_file += str(seed) + "_"    
    dir_file += model_filename
